...
//				std::cout << typeStr << " : " << varName << " : " << xmlSpec << std::endl;
				...
				for (unsigned int i=1; i<tokens.size(); ++i) {
					if (tokens[i] == "required") {
						...
						std::string alternativeName = tokens[i].substr(4);
						// we will use this tag name instead
						xmlInfo.alternativeTagName = alternativeName;
					}
					else if (tokens[i].find("precision=") == 0) {
						std::string prec = tokens[i].substr(10);
						// store precision info
						xmlInfo.numberPrecision = IBK::string2val<unsigned int>(prec);
					}
					else {
