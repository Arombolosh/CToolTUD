			...
			for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
					...
					xmlInfo.typeStr == "bool")
				{
					...
					if (xmlInfo.typeStr == "unsigned int" || xmlInfo.typeStr == "IDType") {
						attribs += "	if (m_"+attribName+" != INVALID_ID)\n	";
						includes.insert("CodeGen_Constants.h");
					}
					...
					attribs +=
							"	e->SetAttribute(\""+attribName+"\", IBK::val2string<double>(m_"+attribName+".m_x) + \" \" + IBK::val2string<double>(m_"+attribName+".m_y));\n";
				}
				else {
					...
					throw IBK::Exception(IBK::FormatString("(Still) unsupported XML attrib type '%1' of variable '%2' in writeXML.")
			...
			for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
					...
					xmlInfo.typeStr == "bool")
				{
					if (xmlInfo.typeStr == "unsigned int") {
						elements += "	if (m_"+varName+" != INVALID_ID)\n	";
						includes.insert("CodeGen_Constants.h");
					}
					...
					elements +=
							...
							"		TiXmlElement::appendSingleAttributeElement(e, \""+tagName+"\", nullptr, std::string(), IBK::val2string<unsigned int>(m_"+varName+".toJulianDay()));\n";
				}
				else if (xmlInfo.typeStr == "IBK::MultiLanguageString") {
					elements +=
							"	if (!m_" + varName + ".empty())\n"
...
//							"			IBK_ASSERT(\""+tagName+"\" == m_"+varName+"[i].m_name);\n"
				...
				//write vectors
					...
					if (childType == "double" || childType == "int" || childType == "unsigned int") {
						...
						includes.insert("CodeGen_Utilities.h");
						// we generate the parent element and afterwards the loop
						elements += "	CodeGen::writeVector2D(e, \""+tagName+"\", m_"+varName+");\n";
					}
					...
					if (childType == "double" || childType == "int" || childType == "unsigned int") {
						includes.insert("CodeGen_Utilities.h");
						// we generate the parent element and afterwards the loop
						elements += "		CodeGen::writeVector(e, \""+tagName+"\", temp);\n";
					}
					else if (childType == "IBKMK::Vector3D") {
						includes.insert("CodeGen_Utilities.h");
						// we generate the parent element and afterwards the loop
						elements += "		CodeGen::writeVector3D(e, \""+tagName+"\", temp);\n";
					}
					else {
						// we generate the parent element and afterwards the loop
						elements += "\n"
					...
					elements += "	} // closing brace to destroy 'temp' again\n";
				...
				else if (xmlInfo.typeStr.find("IBK::point3D<") == 0) {
					// extract subtype
					std::string::size_type pos1 = xmlInfo.typeStr.find("<");
					std::string::size_type pos2 = xmlInfo.typeStr.find(">");
					if (pos2 == std::string::npos)
					...
					// special handling for vector of double or vector of int/unsigned int
					if (childType == "int" || childType == "unsigned int") {
						includes.insert("CodeGen_Utilities.h");
						elements += "	CodeGen::writePoint3D(e, \""+tagName+"\", m_"+varName+");\n";
					}
					else if (childType == "double") {
						includes.insert("CodeGen_Utilities.h");
						elements += "	CodeGen::writePoint3D(e, \""+tagName+"\", m_"+varName+"," + IBK::val2string(xmlInfo.numberPrecision)+");\n";
					}
					else
						throw IBK::Exception(IBK::FormatString("Unsupported template type in variable type '%1' of variable '%2' in writeXML.")
							 .arg(xmlInfo.typeStr).arg(xmlInfo.varName), FUNC_ID);
			...
			if (haveAttribs) {
				...
				for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
						...
						xmlInfo.typeStr == "bool")
					{
						...
						attribs +=
							"				m_"+attribName+" = CodeGen::readVector2DAttribute(element, attrib);\n";
						includes.insert("CodeGen_Utilities.h");
						elseStr = "else ";
					}
					else if (xmlInfo.typeStr == "std::string") {
			...
			if (haveTags) {
				...
				for (const ClassInfo::XMLInfo & xmlInfo : ci.m_xmlInfo) {
						...
						xmlInfo.typeStr == "bool")
					{
						...
						elements +=
							"			"+elseStr+"if (cName == \""+tagName+"\")\n"
							"				m_"+varName+" = QDate::fromJulianDay(QString::fromStdString(c->GetText()).toUInt());\n";
						handledVariables.insert(varName);
					}
					else if (xmlInfo.typeStr == "IBK::MultiLanguageString") {
						elements +=
					...
					// for the group types, we generate the code only once and switch the parameter in an internal loop
						...
						includes.insert("NANDRAD_LinearSplineParameter.h");
					...
					// *** end groups (static array) handling
						...
						if (childType == "double" || childType == "int" || childType == "unsigned int") {
							...
							elements +=
									"			"+elseStr+"if (cName == \""+tagName+"\")\n"
									"				CodeGen::readVector2D(c, \""+tagName+"\", m_"+varName+");\n";
						}
						else if (childType == "IBKMK::Vector3D") {
							// we generate the parent element and afterwards the loop
							elements +=
							...
							// FIXME: hardcoding the libraries namespaces is evil, this should be parametrized

							// remove m_namespace+":: prefix
							if (childTagName.find(m_namespace+"::") == 0)
								childTagName = childTagName.substr(m_namespace.size()+2);
							// remove VICUS:: prefix
					...
					// for std::set, create a vector and load the data into there and transfer to set
						...
						if (childType == "double" || childType == "int" || childType == "unsigned int") {
							...
							elements +=
									...
									"				CodeGen::readVector(c, \""+tagName+"\", temp);\n";
						}
						else if (childType == "IBKMK::Vector3D") {
							// we generate the parent element and afterwards the loop
							elements +=
									...
									"				// create a vector to read the data\n"
									"				std::vector<"+childType+"> temp;\n"
									"				CodeGen::readVector3D(c, \""+tagName+"\", temp);\n";
						}
						else {
							// *** SPECIAL HANDLING FOR NANDRAD AND VICUS NAMESPACES ***
					...
					else if (xmlInfo.typeStr.find("IBK::point3D<") != std::string::npos) {
						...
						if (childType == "double" || childType == "int" || childType == "unsigned int") {
							...
							elements +=
									"			"+elseStr+"if (cName == \""+tagName+"\")\n"
									"				CodeGen::readPoint3D(c, \""+tagName+"\", m_"+varName+");\n";
						}
						// error handling already in read code
			...
			includes.erase("CodeGen_Constants.h"); // is always added anyways
...
void CodeGenerator::generateKeywordlistCode(const IBK::Path & keywordListCpp) {
	...
	if (catnames.empty()) {
		IBK::IBK_Message("No categories extracted. Keywordlist.cpp will not be generated.", IBK::MSG_WARNING, FUNC_ID);
		return;
	}
