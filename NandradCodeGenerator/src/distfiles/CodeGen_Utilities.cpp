...
{
	FUNCID(CodeGen::readLinearSplineElement);
...
void readParameterElement(const TiXmlElement * element, IBK::Parameter & p) {
	FUNCID(CodeGen::readParameterElement);
...
void readIntParaElement(const TiXmlElement * element, IBK::IntPara & p) {
	FUNCID(CodeGen::readIntParaElement);
	const TiXmlAttribute* attrib = TiXmlAttribute::attributeByName(element, "name");
	if (attrib == nullptr)
...
void readFlagElement(const TiXmlElement * element, IBK::Flag & f) {
	FUNCID(CodeGen::readFlagElement);
	std::string namestr;
	std::string valueStr;
...
IBK::Unit readUnitElement(const TiXmlElement * element, const std::string & eName) {
	FUNCID(CodeGen::readUnitElement);
	try {
...
IBK::Time readTimeElement(const TiXmlElement * element, const std::string & eName) {
	FUNCID(CodeGen::readTimeElement);
...
template<>
void readPoint3D<double>(const TiXmlElement * element, const std::string & name, IBK::point3D<double> & p) {
	FUNCID(CodeGen::readVector);
	std::string text = element->GetText();
	try {
		std::vector<double> vec;
...
void writePoint3D(TiXmlElement * parent, const std::string & name, const IBK::point3D<double> & p, unsigned int precision) {
...
void readVector2D(const TiXmlElement * element, const std::string & name, std::vector<IBKMK::Vector2D> & vec) {
	...
	try {
		...
		vec.resize(vals.size() / 2);
		for (unsigned int i=0; i<vec.size(); ++i)
			vec[i].set(vals[i*2], vals[i*2+1]);
...
IBKMK::Vector2D readVector2DAttribute(const TiXmlElement * element, const TiXmlAttribute * attrib) {
	FUNCID(CodeGen::readVector2DAttribute);
	std::stringstream strm(attrib->ValueStr());
	IBKMK::Vector2D vec;
	if (!(strm >> vec.m_x >> vec.m_y))
		throw IBK::Exception( IBK::FormatString(XML_READ_ERROR).arg(element->Row()).arg(
