...
public:
	...
	struct XMLInfo {
		...
		unsigned int	numberPrecision = 0; // If set, some double write functions use this value for precision
	};
	...
	/*! Set to true if CODEGEN_READWRITE_PRIVATE macro is found in file. */
	bool					m_requirePrivateReadWrite = false;
	/*! Set to true, if CODEGEN_COMP macro is found in file. */
