...
std::vector<Table*> Table::fittingTables(QPaintDevice* paintDevice, qreal hfirst, qreal hrest) {
	...
	for(unsigned int i=0; i<tableRows.size(); ++i) {
		unsigned int endRow = tableRows[i];
...
void Table::frameRect(Table& table, int cLeft, int cRight, int rTop, int rBottom, int lineWidth) {
	for( int i=rTop; i<=rBottom; ++i) {
		table.cell((unsigned int)cLeft, i).setBorderWidth(QtExt::TableCell::LeftBorder, lineWidth);
		table.cell((unsigned int)cRight, i).setBorderWidth(QtExt::TableCell::RightBorder, lineWidth);
	}
	for( int i=cLeft; i<=cRight; ++i) {
		table.cell(i, (unsigned int)rTop).setBorderWidth(QtExt::TableCell::TopBorder, lineWidth);
		table.cell(i, (unsigned int)rBottom).setBorderWidth(QtExt::TableCell::BottomBorder, lineWidth);
	}
}
