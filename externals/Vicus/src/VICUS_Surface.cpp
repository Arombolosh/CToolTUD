		...
		if (cName == "Polygon3D") {
			try {
				...
				IBK::IBK_Message( IBK::FormatString(XML_READ_ERROR).arg(element->Row()).arg(
					IBK::FormatString("Error reading Polygon3D tag: %1").arg(e.what()) ), IBK::MSG_ERROR, FUNC_ID);
			}
...
#ifdef HOLE_FIX
	// *** FIX HOLES ***+
	for (PlaneGeometry::Hole &h : holes) {

		std::vector<IBKMK::Vector2D> holeVerts = h.m_holeGeometry.vertexes();

		for	(int idx = 0; idx < holeVerts.size(); ++idx) {
			IBKMK::Vector2D &p1 = holeVerts[ idx						];
			// IBKMK::Vector2D &p2 = holeVerts[(idx + 1) % holeVerts.size()];

			const IBKMK::Polygon2D &poly2D = geometry().polygon2D();

			unsigned int i = 0;
			if (IBKMK::pointInPolygon(poly2D.vertexes(), p1, &i) == 0) { // on polyline
				const IBKMK::Vector2D &v1 = poly2D.vertexes()[ i								 ];
				const IBKMK::Vector2D &v2 = poly2D.vertexes()[(i + 1) % poly2D.vertexes().size() ];

				IBKMK::Vector2D v = (v2 - v1).normalized();

				IBKMK::Vector2D rectVec (-v.m_y, v.m_x);

				int signs[] = {-1, 1};

				double TOL = 1e-2;
				for (int sign : signs) {
					IBKMK::Vector2D p1New = p1 + sign * TOL * rectVec;
					if (IBKMK::pointInPolygon(poly2D.vertexes(), p1New) == -1)
						continue;
		...
		h.m_holeGeometry.setVertexes(holeVerts);
	}
	// *** FIX HOLES END ***
#endif
