...
public:
	...
	/*! Component types.
		Each component instance can be classified based on its location in the building into the following types.
		These types can be used to filter/select component instances by usage.

		When a component instance is newly create (as part of a zone), the component type is automatically selected
		based on inclination.
		When a wall component instance is connected between two zones (via manual/auto connection), its type is changed
		from outside to inside wall.
