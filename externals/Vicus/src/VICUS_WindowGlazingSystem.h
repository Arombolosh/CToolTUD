...
public:
	...
	enum para_t {
		...
		P_SHGCHemis,					// Keyword: SHGCHemis					[---]		'Short wave transmittance across glazing system for diffuse (hemispherical) radiation.'
		NUM_P
	};
	...
	*/
	...
	/*! Returns the calculated or given SHGC for perpendicular incidence (angle 0) depending on model type. [---].
		Only call this function for valid data, otherwise return value is undefined (-1).
		\note This SHGC-value is supposed to be used for the direct radiation part, only.
		If a hemispherical value is
	*/
	double SHGC() const;
