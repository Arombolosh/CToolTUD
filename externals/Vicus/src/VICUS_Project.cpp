...
void Project::readXML(const IBK::Path & filename, IBK::Version & srcVersion) {
	...
	bool migrated = Migration::migrateProject(xmlElem, srcVersion, *this);
	if (migrated) {
...
void Project::readXMLDocument(TiXmlElement * rootElement) {
	...
	try {
			...
			for (VICUS::BuildingLevel & bl : b.m_buildingLevels) {
				for (VICUS::Room &r : bl.m_rooms) {
					for (VICUS::Surface &s : r.m_surfaces) {
						...
						s.m_frontColor = s.m_displayColor;
						s.m_backColor = s.m_displayColor.darker();
		...
		for (VICUS::Surface &s : m_plainGeometry.m_surfaces) {
			if (s.m_frontColor == QColor::Invalid) {
				s.m_frontColor = QColor("#C0C0C0");
			}
			if (s.m_backColor == QColor::Invalid) {
				s.m_backColor = QColor("#C0C0C0");
			}
		}
...
void Project::updatePointers() {
		...
		for (VICUS::BuildingLevel & bl : b.m_buildingLevels) {
			...
			for (VICUS::Room & r : bl.m_rooms) {
				...
				for (VICUS::Surface & s : r.m_surfaces) {
					...
					for (VICUS::Hole & h : const_cast<std::vector<VICUS::Hole> &>(s.holes()) ) {
						addAndCheckForUniqueness(&h);
					}
				}
			}
		}
	...
	std::vector<const VICUS::Bitmap *> bitmaps;
	for (VICUS::Bitmap &bm: m_bitmaps) {
		addAndCheckForUniqueness(&bm);
		bitmaps.push_back(&bm);
	}

	// Has to be called for field size update
	if (!drawings.empty()) {
		IBKMK::Vector3D center;

		// MIND: Needed for field size update and faster picking with drawings
		boundingBox(bitmaps, drawings, std::vector<const Surface *> (), std::vector<const SubSurface *> (),
					std::vector<const NetworkEdge *> (), std::vector<const NetworkNode *> (), center);
	}
							...
							bool takeSelected, bool takeVisible) const
{
			...
			for (const VICUS::BuildingLevel & bl : b.m_buildingLevels) {
				for (const VICUS::Room & r : bl.m_rooms) {
					for (const VICUS::Surface & s : r.m_surfaces) {
						...
						for (const VICUS::Hole & h : s.holes()) {
							if (selectionCheck(h, takeSelected, takeVisible))
								selectedObjs.insert(&h);
						}
					}
	...
	if (sg & SG_Drawing) {
		...
		for (const VICUS::Bitmap &bm : m_bitmaps) {
			if (selectionCheck(bm, takeSelected, takeVisible))
				selectedObjs.insert(&bm);
		}
	}
...
IBKMK::Vector3D Project::boundingBox(const std::vector<const Bitmap *> &bitmaps,
									 const std::vector<const Drawing *> &drawings,
									 ...
									 IBKMK::Vector3D &center)
{
	...
	// store selected surfaces
	if ( surfaces.empty() && subsurfaces.empty() && drawings.empty() && bitmaps.empty() && edges.empty() && nodes.empty() )
	...
	for (const VICUS::Bitmap *bm : bitmaps) {
		bm->m_rectangle.boundingBox(lowerValues, upperValues);
	}
	...
	for (const VICUS::Drawing *drawing : drawings) {
		...
		if (drawing->dirtyPickPoints()) {
			drawing->m_boundingBox = upperValues - lowerValues;
			drawing->m_fieldSize = std::fmax(drawing->m_boundingBox.m_x, drawing->m_boundingBox.m_y) / 100;
		}
	}
...
IBKMK::Vector3D Project::boundingBox(const std::vector<const NetworkEdge *> & edges,
									 const std::vector<const NetworkNode *> & nodes,
									 IBKMK::Vector3D & center) {
									 ...
									 const IBKMK::Vector3D & zAxis)
{
	...
	// store selected surfaces
	if ( surfaces.empty() && subsurfaces.empty() && drawings.empty() && bitmaps.empty() && nodes.empty() && edges.empty())
		return IBKMK::Vector3D (0.,0.,0.);
	...
	for (const VICUS::Bitmap *bm : bitmaps) {
		for (IBKMK::Vector3D v : bm->m_rectangle.vertexes()) {
			IBKMK::Vector3D vLocal, point;
	...
	for (const VICUS::Drawing *d : drawings ) {
		...
		if (d->dirtyPickPoints()) {
			d->m_boundingBox = upperValues - lowerValues;
			d->m_fieldSize = std::fmax(d->m_boundingBox.m_x, d->m_boundingBox.m_y) / 100;
		}
	}
...
void Project::generateNandradProject(NANDRAD::Project & p, QStringList & errorStack, QStringList & warningStack, const std::string & nandradProjectPath) const {
	...
	try {
		generateBuildingProjectData(QString(IBK::Path(nandradProjectPath).filename().withoutExtension().c_str()),
									p, errorStack, warningStack, surfaceIdsVicusToNandrad, roomMappings, componentInstanceMappings);
	}
	catch(IBK::Exception &ex) {
