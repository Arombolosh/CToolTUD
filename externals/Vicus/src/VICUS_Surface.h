...
public:
	...
	*/
	...
	/*! Color to be used when next updating the geometry.
		...
		In order to make a darker color of the side, that is oposite of the normal, we need a front and a back color.
		This is only used in add geometry mode currently.

		Color is not a regular property of a surface, but rather of the associated parameter elements.
	*/
	mutable QColor						m_backColor; // Note: mutable so that it can be modified on const project

	/*! ToDo Andreas: Is this okay for you?
		Added a backcolor for normal geometry mode to display a slightly darker color - so that one can determine
		the surface normal without surface normal displaying mode.
	*/
	mutable QColor						m_frontColor; // Note: mutable so that it can be modified on const project
...
private:
	/*! Subsurfaces of the surface, cuts holes in m_geometry. */
	std::vector<SubSurface>				m_subSurfaces;				// XML:E

	/*! Holes inside the surface. Needed for smart-trimming or breaches in walls. Also cuts holes in m_geometry. */
