...
const std::vector<Drawing::LineSegment> &Drawing::Point::lineGeometries() const {
	...
	try {
		...
		// 	m_lineGeometries.push_back(VICUS::PlaneGeometry(po));
		// 	m_dirtyGlobalPoints = false;
		// }
...
void Drawing::generateLinesFromText(const std::string &text, double textHeight, Qt::Alignment alignment,
	...
	double scalingFactorFonts = std::min(textHeight * DEFAULT_FONT_SCALING * m_scalingFactor, DEFAULT_FONT_SIZE * m_scalingFactor);
	...
	for (int i=0; i < polygons.size(); ++i) {
		...
		for (unsigned int i=0; i<poly.size(); ++i) {
			...
			poly[i] = IBKMK::Vector2D(  point.x() * m_textScaling * scalingFactorFonts + basePoint.m_x,
										-point.y() * m_textScaling * scalingFactorFonts + basePoint.m_y);
		}

		IBKMK::Polygon3D poly3D;
		try {
			poly3D = IBKMK::Polygon3D (points3D(poly, object));
		} catch (IBK::Exception &) {
			IBK::IBK_Message(IBK::FormatString("Could not render text '%1'. Skipping").arg(text), IBK::MSG_WARNING);
			return;
		}
		...
		for (unsigned int i = 0; i < poly3D.vertexes().size(); ++i) {
			const IBKMK::Vector3D &v1 = poly3D.vertexes()[ i							  ];
			const IBKMK::Vector3D &v2 = poly3D.vertexes()[(i+1) % poly3D.vertexes().size()];
			lineGeometries.push_back(LineSegment(v1, v2));
		}
	}
}
