	...
	switch (m_modelType) {
		case MT_Simple: {
			try {
				...
				m_para[P_SHGCHemis].checkedValue("SHGCHemis", "---", "---", 0, true, 1, true, nullptr);
			}  catch (...) {
				return false;
			}
		}
	...
	switch (m_modelType) {
		case MT_Simple: {
			try {
				...
				// TODO : check for valid x-value range!
				//        This is tricky, as we need to check either stored values or read the values from externally referenced tsv file
//				if (spl.m_values.x().front() != 0) || spl.m_values.x().back() != 90)
//					return false;
			}  catch (...) {
				return false;
			}
