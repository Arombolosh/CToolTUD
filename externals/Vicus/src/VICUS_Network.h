...
public:
	...
	*/
	...
	/*! Add mixer node to network based on coordinates and return the node id.
	 *  When given coordinates exist already in the network: return the id of this existing node, but never change its type. */
	unsigned int addMixerNodeKeepExistingType(unsigned int preferedId, const IBKMK::Vector3D &v);
	...
	*/
	...
	/*! Assign heating demands to all edges according to the connected buildings and the shortest paths from buildings to sources. */
	void setEdgeNominalHeatingDemands();
