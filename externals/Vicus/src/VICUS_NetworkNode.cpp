...
void NetworkNode::adjacentHeatingDemand(std::set<NetworkEdge *> &visitedEdges, std::vector<double> &adjacentDemands){
	for (NetworkEdge *e: m_edges){
		if (visitedEdges.find(e) == visitedEdges.end()){
			visitedEdges.insert(e);
			if (e->m_nominalHeatingDemand > 0)
				adjacentDemands.push_back(e->m_nominalHeatingDemand);
		}
	}
}
