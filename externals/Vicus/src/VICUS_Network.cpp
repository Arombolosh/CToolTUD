...
unsigned int Network::addMixerNodeKeepExistingType(unsigned int preferedId, const IBKMK::Vector3D & v) {
	for (NetworkNode &n: m_nodes){
		if (n.m_position.distanceTo(v) < NetworkGeometricResolution){
			return n.m_id;
		}
	}
	m_nodes.push_back(NetworkNode(preferedId, NetworkNode::NT_Mixer, v));
	updateNodeEdgeConnectionPointers();
	return preferedId;
}
		...
		for (unsigned i1=0; i1<m_edges.size(); ++i1) {
			for (unsigned i2=i1+1; i2<m_edges.size(); ++i2) {
				...
				if (l1.containsPoint(ps) && l2.containsPoint(ps)){
					// add the cross point
					unsigned nCross = addNode(++nextUnusedId, ps, NetworkNode::NT_Mixer);
					bool crossPointInEdge1 = nCross == m_edges[i1].nodeId1() || nCross == m_edges[i1].nodeId2();
					bool crossPointInEdge2 = nCross == m_edges[i2].nodeId1() || nCross == m_edges[i2].nodeId2();
					// if in both edges, we have two line end points "crossing", we do nothing
					if (crossPointInEdge1 && crossPointInEdge2)
						continue;
					// if only in edge1, split edge2
					if (crossPointInEdge1) {
						addEdge(++nextUnusedId, nCross, m_edges[i2].nodeId1(), true, m_edges[i2].m_idPipe);
						m_edges[i2].changeNode1(nodeById(nCross));
					}
					// if only in edge2, split edge1
					else if (crossPointInEdge2) {
						addEdge(++nextUnusedId, nCross, m_edges[i1].nodeId1(), true, m_edges[i1].m_idPipe);
						m_edges[i1].changeNode1(nodeById(nCross));
					}
					// split both edges
					else {
						addEdge(++nextUnusedId, nCross, m_edges[i1].nodeId1(), true, m_edges[i1].m_idPipe);
...
void Network::setEdgeNominalHeatingDemands() {
	FUNCID(Network::setEdgeNominalHeatingDemands);
	...
	// for that, we look for edges that have no heating demand assigned yet. We then collect the demands of neighboring edges if these are > 0.
	// if no direct neighbors with heating demand > 0 are found we might need to repeat the procedure several times
	bool repeatFindAdjacent = true;
	while (repeatFindAdjacent) {
		repeatFindAdjacent = false;
		for (NetworkEdge &e: m_edges){
			if (e.m_nominalHeatingDemand <= 0){
				std::set<NetworkEdge *> edges;
				std::vector<double> adjacentDemands;
				e.m_node1->adjacentHeatingDemand(edges, adjacentDemands);
				e.m_node2->adjacentHeatingDemand(edges, adjacentDemands);
				if (adjacentDemands.size() > 0)
					e.m_nominalHeatingDemand = std::accumulate(adjacentDemands.begin(), adjacentDemands.end(), 0) / (double)adjacentDemands.size();
				else
					repeatFindAdjacent = true;
			}
		}
	}
}
...
FUNCID(Network::sizePipeDimensions);
	...
	// find shortest paths and assign heating demands to the according edges
	setEdgeNominalHeatingDemands();
