...
public:
	...
	void generate(const std::vector<ComponentInstance> &componentInstances, std::map<unsigned int, unsigned int> &componentInstanceMappings,
				  QStringList &errorStack, QStringList &warningStack, std::set<unsigned int> &idSet);
...
private:
	...
	NANDRAD::Interface generateInterface(const VICUS::ComponentInstance & ci, unsigned int bcID,
										 ...
										 const NANDRAD::Interface::usage_t & usageType,
	...
	/*! Generates the usage type for a surface.
		Inclination < 60° -> roof
		145 < inclination < 181° -> floor
		Inclination > 181° -> Other
		otherwise wall
	*/
	NANDRAD::Interface::usage_t usageType(double inclination);
...
void Project::generateBuildingProjectData(const QString &modelName, NANDRAD::Project & p, QStringList & errorStack,QStringList & warningStack,
	...
	constrInstaModelGenerator.generate(m_componentInstances, componentInstanceMapping, errorStack, warningStack, idSet);
																		  ...
																		  bool takeASide) const
{
	...
	if (s != nullptr) {
		...
		// for outside zone write other type
		if(iface.m_zoneId == 0)
			iface.m_usage = NANDRAD::Interface::U_Other;
		else
			iface.m_usage = usageType;
		return iface;
	}
	else {
		...
		// for outside zone write other type
		if(iface.m_zoneId == 0 || hasGroundContact)
			iface.m_usage = NANDRAD::Interface::U_Other;
		else
			iface.m_usage = usageType;
		return iface;
...
NANDRAD::Interface::usage_t ConstructionInstanceModelGenerator::usageType(double inclination){
	// Roof/ceiling
	if(inclination <= 60)
		return NANDRAD::Interface::U_Ceiling;
	else if(inclination > 181)
		return NANDRAD::Interface::U_Other;
	else if (inclination > 145)
		return NANDRAD::Interface::U_Floor;
	else
		return NANDRAD::Interface::U_Wall;
}

void ConstructionInstanceModelGenerator::generate(const std::vector<ComponentInstance> &componentInstances,
												  std::map<unsigned int, unsigned int> &compInstMapping, QStringList &errorStack,
												  QStringList &warningStack, std::set<unsigned int> &idSet) {
	...
	for (const VICUS::ComponentInstance & compInstaVicus : componentInstances) {
		...
		const double SURFACE_AREA_ABS_TOL = 0.25; // in m;
		double area = 0;
		...
		double sideAInclination = -1;
		double sideBInclination = -1;

		bool bothSidesHaveSurfaces = false;
		// we have either one or two surfaces associated
		if (compInstaVicus.m_sideASurface != nullptr) {
			sideAInclination = compInstaVicus.m_sideASurface->geometry().inclination();
			...
			if (compInstaVicus.m_sideBSurface != nullptr) {
				...
				sideBInclination = compInstaVicus.m_sideBSurface->geometry().inclination();
				// have both
...
// #define ONLY_ABS_TEST
#ifdef ONLY_ABS_TEST
				if (std::fabs(area - areaB) > SAME_DISTANCE_PARAMETER_ABSTOL) {
#else
				const double SURFACE_AREA_REL_TOL = 0.05;
				// relative error check
				// area is set as reference
				if ((std::fabs(area - areaB) > SURFACE_AREA_ABS_TOL) && (std::fabs(areaB-area)/area > SURFACE_AREA_REL_TOL) ) {
#endif
					warnings.append(qApp->tr("Component/construction #%1 references surfaces #%2 '%6' and #%3 '%7', with mismatching "
											   "areas %4 m² and %5 m².")
			...
			if(area<MIN_AREA_FOR_EXPORTED_SURFACES){
				warnings << qApp->tr("Area of surface '%2' (#%1) is too small (%3 m²). Surface is not exported.")
							.arg(compInstaVicus.m_sideASurface->m_id)
							.arg(compInstaVicus.m_sideASurface->m_displayName)
							.arg(compInstaVicus.m_sideASurface->areaWithoutSubsurfaces());
				continue;
			}
		...
		NANDRAD::Interface::usage_t useType = usageType(sideAInclination);
		constrInstNandrad.m_interfaceA = generateInterface(compInstaVicus, comp->m_idSideABoundaryCondition, 1, errorStack, gze, useType, true);
		...
		useType = usageType(sideBInclination);
		constrInstNandrad.m_interfaceB = generateInterface(compInstaVicus, comp->m_idSideBBoundaryCondition, 2, errorStack, gze2, useType, false);
	...
	for (const QString &str : warnings) {
		...
		warningStack.insert(warningStack.end(), str);
	}
...
void ConstructionInstanceModelGenerator::generateConstructions(QStringList &errorStack) {
	...
	for (const VICUS::WindowGlazingSystem &w : m_project->m_embeddedDB.m_windowGlazingSystems){
		...
		NANDRAD::KeywordList::setParameter(winG.m_para, "WindowGlazingSystem::para_t",
										   ...
										   w.uValue());
		NANDRAD::KeywordList::setParameter(winG.m_para, "WindowGlazingSystem::para_t",
										   NANDRAD::WindowGlazingSystem::P_SHGCHemis,
										   w.m_para[VICUS::WindowGlazingSystem::P_SHGCHemis].value);
...
void SupplySystemNetworkModelGenerator::generate(const SupplySystem & supplySystem,
	...
	// If Vicus SubNetwork is given: transfer it into our nandrad network
		...
		// *** Transfer elements
		std::map<unsigned int, std::vector<unsigned int>> component2ElementsMap;
		std::map<unsigned int, std::vector<unsigned int>> controller2ElementsMap;
		for (const NetworkElement &elem: subNetwork->m_elements) {
			...
			// remember elements for each controller which has schedules (for object list)
			const VICUS::NetworkController *ctr = dbNetworkCtrl[elem.m_controlElementId];
			if (ctr != nullptr){
				unsigned int idSchedule = ctr->m_idReferences[NetworkController::ID_Schedule];
				if (ctr->m_modelType == NetworkController::MT_Scheduled && idSchedule != INVALID_ID)
					controller2ElementsMap[elem.m_controlElementId].push_back(nandradElement.m_id);
			}

			// remember elements for each component, which has schedules (for object list)
			if (!comp->m_scheduleIds.empty())
				component2ElementsMap[elem.m_componentId].push_back(nandradElement.m_id);
		...
		// -> Transfer schedules of components
		for(auto it=component2ElementsMap.begin(); it!=component2ElementsMap.end(); ++it){
		...
		// -> Transfer schedules of controllers
		for(auto it=controller2ElementsMap.begin(); it!=controller2ElementsMap.end(); ++it){

			const VICUS::NetworkController *ctr = dbNetworkCtrl[it->first];
			unsigned int schedId = ctr->m_idReferences[NetworkController::ID_Schedule];
			Q_ASSERT(ctr != nullptr && schedId != INVALID_ID);
			...
			// get the required schedule name, which depends on the property that is being controlled
			Q_ASSERT((unsigned int)VICUS::NetworkController::ControlledProperty::NUM_CP ==
					 (unsigned int)NANDRAD::HydraulicNetworkControlElement::ControlledProperty::NUM_CP);
			unsigned int ctrProp = (unsigned int)ctr->m_controlledProperty;
			std::string schedName =NANDRAD::HydraulicNetworkControlElement::requiredScheduleName(
											NANDRAD::HydraulicNetworkControlElement::ControlledProperty(ctrProp));
			Q_ASSERT(!schedName.empty());

			// create object list
			NANDRAD::ObjectList objList;
			std::string objListName = "network elements with " + sched->m_displayName.string();
			bool createNewObjList = true;
			// Note: we cannot have 2 object lists with same name
			// if we have a object list with identical name already, just add the ids to that
			for (NANDRAD::ObjectList &objList: m_objLists) {
			...
			m_scheduleGroups[objListName] = std::vector<NANDRAD::Schedule>();
			std::vector<NANDRAD::LinearSplineParameter> splines;
			sched->insertIntoNandradSchedulegroup(schedName, m_scheduleGroups[objListName], splines, m_placeholders);
			// annual schedules handled separately
			if (!splines.empty())
				m_annualSchedules[objListName].push_back(splines[0]);
...
void Project::generateNetworkProjectData(NANDRAD::Project & p, QStringList &errorStack, const std::string & nandradProjectPath, unsigned int networkId) const {
	...
	std::map<unsigned int, std::vector<unsigned int>> componentElementMap; // stores the NANDRAD element ids for each NANDRAD/VICUS component id
	std::map<unsigned int, std::vector<unsigned int>> controller2ElementsMap; // stores the NANDRAD element ids for each controller id
	...
	for (const VICUS::NetworkNode &node: vicusNetwork.m_nodes) {
		...
		for (const NetworkElement &elem: sub->m_elements){
			...
			// also remember elements for each controller which has schedules (for object list)
			const VICUS::NetworkController *ctr = dbNetworkCtrl[elem.m_controlElementId];
			if (ctr != nullptr){
				unsigned int idSchedule = ctr->m_idReferences[NetworkController::ID_Schedule];
				if (ctr->m_modelType == NetworkController::MT_Scheduled && idSchedule != INVALID_ID)
	...
	std::map<unsigned int, NANDRAD::ObjectList> objectListMapComponents;
	for (auto it=componentElementMap.begin(); it!=componentElementMap.end(); ++it){
		...
		objectListMapComponents[comp->m_id] = p.m_objectLists.back();
	}
	...
	for (unsigned int compId: componentIdsVicus){
		...
		const NANDRAD::ObjectList &objList = objectListMapComponents[comp->m_id];
	...
	// *** Transfer SCHEDULES for each VICUS controller
	...
	for(auto it=controller2ElementsMap.begin(); it!=controller2ElementsMap.end(); ++it){
		...
		// create object list
		NANDRAD::ObjectList newObjList;
		std::string objListName = "network elements with " + sched->m_displayName.string();
		bool createNewObjList = true;
		// Note: we cannot have 2 object lists with same name
		// if we have a object list with identical name already, just add the ids to that
		for (NANDRAD::ObjectList &objList: p.m_objectLists) {
			if (objList.m_name == objListName) {
				for (unsigned int elemId: it->second)
					objList.m_filterID.m_ids.insert(elemId);
				createNewObjList = false;
		...
		if (createNewObjList) {
			// if not: create new object list
			newObjList.m_name = objListName;
			newObjList.m_referenceType = NANDRAD::ModelInputReference::MRT_NETWORKELEMENT;
			for (unsigned int elemId: it->second)
				newObjList.m_filterID.m_ids.insert(elemId);
			p.m_objectLists.push_back(newObjList);
		}
		...
		if (sched->m_haveAnnualSchedule){
			// copy schedule and check it
			NANDRAD::LinearSplineParameter annualSched = sched->m_annualSchedule;
			p.m_schedules.m_annualSchedules[newObjList.m_name].push_back(annualSched);
		}
		else
			addVicusScheduleToNandradProject(*sched, schedName, p.m_schedules, newObjList.m_name);
	}
