...
class Project {
	...
	*/
	void generateNandradProject(NANDRAD::Project & p, QStringList & errorStack, QStringList &warningStack, const std::string & nandradProjectPath) const;
//	void generateBuildingProjectData(NANDRAD::Project & p) const;
	...
	*/
	static IBKMK::Vector3D boundingBox(const std::vector<const Bitmap *> &bitmaps,
	...
	*/
	...
	/*! Drawings from dxf files, will be read from external *.vicdraw file */
	std::vector<Drawing>								m_drawings;

	/*! Bitmaps, will be read from external bitmap file */
	std::vector<Bitmap>									m_bitmaps;					// XML:E
	...
	*/
	...
	/*! NOTE: ONLY NEEDED FOR MIGRATION, NEVER USE THIS
		IDs with surfaces, that have to insert a hole in the surface.

		\param key is ID of surface, where child has to cut a hole
		\param value set of ids that were childs of the previous parent surface
	*/
	std::map<unsigned int, std::set<unsigned int>>		m_childSurfaceMapping;
...
private:
	...
	void generateBuildingProjectData(const QString &modelName,
									 NANDRAD::Project & p, QStringList & errorStack, QStringList &warningStack,
