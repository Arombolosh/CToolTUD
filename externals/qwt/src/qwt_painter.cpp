...
template <class T>
static void drawPolylineSplitAtNAN(QPainter *painter, const T *points, const T *pointsEnd) {
    // process points vector until we reach the end
    while (points != pointsEnd) {
        // search for first occurrance of a point without NAN in x or y
        while (points != pointsEnd) {
            if (!(points->x() != points->x() || points->y() != points->y())) break;
            ++points;
        }
        if (points == pointsEnd) break; // done painting
        // search first first occurance of sample with NAN in x or y
        const T *polylineEnd = points+1;
        while (polylineEnd != pointsEnd) {
            if (polylineEnd->x() != polylineEnd->x() || polylineEnd->y() != polylineEnd->y()) break;
            ++polylineEnd;
        }
        // NOTE: when polylineEnd == pointsEnd, we now draw the last segment
        painter->drawPolyline( points, polylineEnd-points );
        points = polylineEnd;
    }
}
    ...
    if ( doSplit )
    {
            ...
            && !( painter->renderHints() & QPainter::Antialiasing ) )
        {
            ...
            for ( int i = k + 1; i < pointCount; i++ )
            {
                ...
                {
                    const T * pointsStart = points + k;
                    const T * pointsEnd = points + i + 1;

                    // now draw the points between pointsStart and pointsEnd as polyline but split
                    // at occurances of NAN in x or y
                    drawPolylineSplitAtNAN(painter, pointsStart, pointsEnd);
                }
            }

            drawPolylineSplitAtNAN(painter, points + k, points + pointCount);
        }
        else
        {
            for ( int i = 0; i < pointCount; i += splitSize )
            {
                ...
                // now draw the points as polyline but split
                // at occurances of NAN in x or y
                drawPolylineSplitAtNAN(painter, points + i, points + i + n);
            }
        }
    }
    else
    {
        // draw the polyline but split at occurances of NAN in x or y
        drawPolylineSplitAtNAN(painter, points, points + pointCount);
    }
}
