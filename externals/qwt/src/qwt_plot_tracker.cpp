...
QwtPlotTracker::LayoutItem::LayoutItem( const QwtPlotTracker* tracker )
    : m_tracker( tracker )
    , m_plotItem( nullptr)
{
}
...
QRegion QwtPlotTracker::trackerMask() const
{
    ...
    if ( !title.isEmpty() )
    {
        ...
        titleHeight = qCeil(titleRect.height() + 2 * m_data->itemMargin);
        titleWidth = qCeil(titleRect.width() + 2 * m_data->itemMargin);
    }

    const QRect rect = trackerRect( QSize( qMax(size.width(), titleWidth), size.height() + titleHeight ) );
