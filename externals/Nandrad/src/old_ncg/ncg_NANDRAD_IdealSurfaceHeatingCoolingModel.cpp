	...
	try {
		...
		while (c) {
			...
			if (cName == "ConstructionObjectList")
				...
				m_thermostatZoneId = CodeGen::readPODElement<unsigned int>(c, cName);
				...
				if (success) {
					std::string refUnit = KeywordList::Unit("IdealSurfaceHeatingCoolingModel::para_t", ptype);
...
TiXmlElement * IdealSurfaceHeatingCoolingModel::writeXML(TiXmlElement * parent) const {
	...
	if (m_thermostatZoneId != INVALID_ID)
