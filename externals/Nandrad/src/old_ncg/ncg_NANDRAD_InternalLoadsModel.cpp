...
void InternalLoadsModel::readXML(const TiXmlElement * element) {
	FUNCID(InternalLoadsModel::readXML);
	...
	try {
		...
		while (c) {
			...
			if (cName == "ZoneObjectList")
				...
				para_t ptype;
				try {
					ptype = (para_t)KeywordList::Enumeration("InternalLoadsModel::para_t", p.name);
					m_para[ptype] = p; success = true;
				}
				catch (...) { /* intentional fail */  }
				if (success) {
					std::string refUnit = KeywordList::Unit("InternalLoadsModel::para_t", ptype);
		...
		throw IBK::Exception( ex, IBK::FormatString("Error reading 'InternalLoadsModel' element."), FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'InternalLoadsModel' element.").arg(ex2.what()), FUNC_ID);
	}
}
...
TiXmlElement * InternalLoadsModel::writeXML(TiXmlElement * parent) const {
	...
	if (m_modelType != NUM_MT)
		e->SetAttribute("modelType", KeywordList::Keyword("InternalLoadsModel::modelType_t",  m_modelType));
