...
void InterfaceSolarAbsorption::readXML(const TiXmlElement * element) {
	FUNCID(InterfaceSolarAbsorption::readXML);
	...
	try {
		...
		while (attrib) {
			...
			if (attribName == "modelType")
				try {
					m_modelType = (modelType_t)KeywordList::Enumeration("InterfaceSolarAbsorption::modelType_t", attrib->ValueStr());
				}
		...
		throw IBK::Exception( ex, IBK::FormatString("Error reading 'InterfaceSolarAbsorption' element."), FUNC_ID);
	}
	catch (std::exception & ex2) {
		throw IBK::Exception( IBK::FormatString("%1\nError reading 'InterfaceSolarAbsorption' element.").arg(ex2.what()), FUNC_ID);
	}
}

TiXmlElement * InterfaceSolarAbsorption::writeXML(TiXmlElement * parent) const {
	TiXmlElement * e = new TiXmlElement("InterfaceSolarAbsorption");
