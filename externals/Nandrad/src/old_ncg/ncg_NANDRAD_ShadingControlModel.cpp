...
void ShadingControlModel::readXML(const TiXmlElement * element) {
	FUNCID(ShadingControlModel::readXML);
	...
	try {
		...
		while (attrib) {
			...
			if (attribName == "id")
				...
				m_sensorId = CodeGen::readPODAttributeValue<unsigned int>(element, attrib);
			else {
...
TiXmlElement * ShadingControlModel::writeXML(TiXmlElement * parent) const {
