...
void HydraulicNetwork::readXML(const TiXmlElement * element) {
	FUNCID(HydraulicNetwork::readXML);
	...
	try {
		...
		while (attrib) {
			...
			if (attribName == "id")
				...
				m_referenceElementId = CodeGen::readPODAttributeValue<unsigned int>(element, attrib);
			else {
		...
		while (c) {
			...
			if (cName == "IBK:Parameter") {
				...
				if (success) {
					std::string refUnit = KeywordList::Unit("HydraulicNetwork::para_t", ptype);
...
TiXmlElement * HydraulicNetwork::writeXML(TiXmlElement * parent) const {
	...
	if (m_referenceElementId != INVALID_ID)
