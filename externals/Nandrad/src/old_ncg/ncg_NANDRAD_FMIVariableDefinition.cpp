	...
	try {
		...
		while (attrib) {
			...
			if (attribName == "fmiVarName")
				...
				m_fmiValueRef = (IDType)CodeGen::readPODAttributeValue<unsigned int>(element, attrib);
			else {
		...
		while (c) {
			...
			if (cName == "FmiVarDescription")
				...
				m_fmiStartValue = CodeGen::readPODElement<double>(c, cName);
			else if (cName == "VarName")
				m_varName = c->GetText();
			else if (cName == "ObjectId")
				m_objectId = (IDType)CodeGen::readPODElement<unsigned int>(c, cName);
			else if (cName == "VectorIndex")
				m_vectorIndex = (IDType)CodeGen::readPODElement<unsigned int>(c, cName);
			else {
