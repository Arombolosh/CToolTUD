	...
	try {
		...
		while (c) {
			...
			if (cName == "ZoneObjectList")
				...
				if (success) {
					std::string refUnit = KeywordList::Unit("Thermostat::para_t", ptype);
				...
				m_referenceZoneId = CodeGen::readPODElement<unsigned int>(c, cName);
...
TiXmlElement * Thermostat::writeXML(TiXmlElement * parent) const {
	...
	if (m_referenceZoneId != INVALID_ID)
