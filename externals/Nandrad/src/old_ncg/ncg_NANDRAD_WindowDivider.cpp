...
void WindowDivider::readXML(const TiXmlElement * element) {
	FUNCID(WindowDivider::readXML);
	...
	try {
		...
		while (attrib) {
			...
			if (attribName == "materialId")
				m_materialId = CodeGen::readPODAttributeValue<unsigned int>(element, attrib);
			else {
