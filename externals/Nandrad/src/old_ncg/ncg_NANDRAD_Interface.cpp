	...
	try {
		...
		while (attrib) {
			...
			if (attribName == "id")
				...
				m_zoneId = CodeGen::readPODAttributeValue<unsigned int>(element, attrib);
			else if (attribName == "usage")
				try {
					m_usage = (usage_t)KeywordList::Enumeration("Interface::usage_t", attrib->ValueStr());
				}
...
TiXmlElement * Interface::writeXML(TiXmlElement * parent) const {
	...
	if (m_usage != NUM_U)
		e->SetAttribute("usage", KeywordList::Keyword("Interface::usage_t",  m_usage));
