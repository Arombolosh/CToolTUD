...
void WindowShading::readXML(const TiXmlElement * element) {
	FUNCID(WindowShading::readXML);
	...
	try {
		...
		while (attrib) {
			...
			if (attribName == "modelType")
				...
				m_controlModelId = CodeGen::readPODAttributeValue<unsigned int>(element, attrib);
			else {
...
TiXmlElement * WindowShading::writeXML(TiXmlElement * parent) const {
