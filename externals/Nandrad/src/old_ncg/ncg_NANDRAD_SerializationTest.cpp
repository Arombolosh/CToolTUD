...
void SerializationTest::readXML(const TiXmlElement * element) {
	...
	try {
		...
		while (attrib) {
			...
			if (attribName == "id1")
				m_id1 = CodeGen::readPODAttributeValue<int>(element, attrib);
			else if (attribName == "id2")
				m_id2 = CodeGen::readPODAttributeValue<unsigned int>(element, attrib);
			else if (attribName == "flag1")
				m_flag1 = CodeGen::readPODAttributeValue<bool>(element, attrib);
			else if (attribName == "val1")
				m_val1 = CodeGen::readPODAttributeValue<double>(element, attrib);
				...
				m_someStuffIDAsAttrib = (IDType)CodeGen::readPODAttributeValue<unsigned int>(element, attrib);
			else {
		...
		while (c) {
			...
			if (cName == "Id3")
				m_id3 = CodeGen::readPODElement<int>(c, cName);
			else if (cName == "Id4")
				m_id4 = CodeGen::readPODElement<unsigned int>(c, cName);
			else if (cName == "Flag2")
				m_flag2 = CodeGen::readPODElement<bool>(c, cName);
			else if (cName == "Val2")
				m_val2 = CodeGen::readPODElement<double>(c, cName);
			else if (cName == "Str2")
				m_str2 = c->GetText();
			else if (cName == "Path2")
				...
				m_u2 = CodeGen::readUnitElement(c, cName);
			else if (cName == "X5")
				m_x5 = CodeGen::readPODElement<double>(c, cName);
				...
				m_time1 = CodeGen::readTimeElement(c, cName);
			else if (cName == "Time2")
				m_time2 = CodeGen::readTimeElement(c, cName);
			else if (cName == "Table")
				m_table.setEncodedString(c->GetText());
			else if (cName == "Table2")
				m_table2.setEncodedString(c->GetText());
			else if (cName == "DblVec")
				CodeGen::readVector(c, "DblVec", m_dblVec);
				...
				m_someStuffIDAsElement = (IDType)CodeGen::readPODElement<unsigned int>(c, cName);
				...
				CodeGen::readPoint2D(c, "Coordinate2D", m_coordinate2D);
				...
				for (int i=0; i<NUM_RefID; ++i) {
					if (cName == KeywordList::Keyword("SerializationTest::ReferencedIDTypes",i)) {
						m_idReferences[i] = (IDType)CodeGen::readPODElement<unsigned int>(c, cName);
						found = true;
						break;
					}
				}
...
TiXmlElement * SerializationTest::writeXML(TiXmlElement * parent) const {
	...
	if (m_someStuffIDAsAttrib != INVALID_ID)
	...
	CodeGen::writeVector(e, "DblVec", m_dblVec);
	...
	if (m_someStuffIDAsElement != INVALID_ID)
	...
	for (int i=0; i<NUM_RefID; ++i) {
		if (m_idReferences[i] != INVALID_ID)
	...
	if (!m_linSpl.empty())
		CodeGen::writeLinearSplineElement(e, "LinSpl", m_linSpl, "-", "-");
	...
	CodeGen::writePoint2D(e, "Coordinate2D", m_coordinate2D);
	return e;
}
