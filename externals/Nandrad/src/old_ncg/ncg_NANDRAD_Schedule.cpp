	...
	try {
		...
		while (c) {
			...
			if (cName == "StartDayOfTheYear")
				m_startDayOfTheYear = CodeGen::readPODElement<unsigned int>(c, cName);
			else if (cName == "EndDayOfTheYear")
				m_endDayOfTheYear = CodeGen::readPODElement<unsigned int>(c, cName);
...
TiXmlElement * Schedule::writeXML(TiXmlElement * parent) const {
	...
	if (m_startDayOfTheYear != INVALID_ID)
	...
	if (m_endDayOfTheYear != INVALID_ID)
