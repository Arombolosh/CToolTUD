...
void WindowGlazingSystem::checkParameters() {
	...
	switch (m_modelType) {
		case MT_Simple : {
			...
			if (m_para[P_SHGCHemis].name.empty())
				m_para[P_SHGCHemis].set( m_splinePara[NANDRAD::WindowGlazingSystem::SP_SHGC].m_values.value(0), IBK::Unit("---"));
			else {
				// check thermal transmittance parameter
				m_para[P_SHGCHemis].checkedValue("SHGCHemis", "---",
												 "---", 0, true, 1, true,
												"Hemisperical SHGC transmittance must be in the range of 0..1.");
			}
		} break;
											 ...
											 bool solarLoadsLeft) const
{
	...
	switch (m_modelType) {
		case MT_Simple : {
			...
			double SHGCHemis = m_para[NANDRAD::WindowGlazingSystem::P_SHGCHemis].value;
