...
std::string HydraulicNetworkControlElement::requiredScheduleName(const ControlledProperty controlledProperty) {
	switch (controlledProperty) {
		case CP_TemperatureDifference:
		case CP_TemperatureDifferenceOfFollowingElement:
			return "TemperatureDifferenceSetpointSchedule [K]";
		case CP_MassFlux:
			return "MassFluxSetpointSchedule [kg/s]";
		case CP_PumpOperation:
		case CP_PressureDifferenceWorstpoint:
		case CP_ThermostatValue:
		case NUM_CP:
