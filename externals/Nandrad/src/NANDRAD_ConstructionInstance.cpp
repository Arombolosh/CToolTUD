...
void ConstructionInstance::checkParameters(const Project & prj) {
	...
	// if we have embedded objects and both sides of the construction are ambient, this is an error as we don't have radiation
	// through embedded windows for free-standing walls
	if (!m_embeddedObjects.empty()) {
		if (m_interfaceA.m_id != INVALID_ID && m_interfaceA.m_zoneId == 0 &&
			m_interfaceB.m_id != INVALID_ID && m_interfaceB.m_zoneId == 0)
		{
			throw IBK::Exception( IBK::FormatString("Construction instance #%1 has embedded objects and on "
													"both sides ambient conditions. This is not supported.").arg(m_id), FUNC_ID);
		}
	}

	// now check that the net heat transfer area = construction instance's area minus embedded object area, does not result in a negative value
	// NOTE: The net heat transfer area may become zero or close to zero. This is explicitly allowed, as it can be a result of disadvantageous geometric modelling.
	// For such cases we just neglect the wall construction instance during model setup but still consider the embedded object.
...
void ConstructionInstance::checkAndPrepareLongWaveHeatExchange(const Project & prj, Interface & ourInterface) {
	...
	for (auto it=ourInterface.m_connectedInterfaces.begin(); it!=ourInterface.m_connectedInterfaces.end(); ++it) {
		...
		if (inter->m_longWaveEmission.m_modelType == InterfaceLongWaveEmission::NUM_MT) {
			throw IBK::Exception( IBK::FormatString("Interface #%1 in construction instance #%2 (%3) exchanges "
													"long wave radiation with interface #%4 in construction instance #%5 (%6), "
													"but it has no long wave emission model defined.")
								  .arg(inter->m_id).arg(connectedConstructionInstances[i]->m_id)
								  .arg(connectedConstructionInstances[i]->m_displayName)
	...
	for (auto it=ourInterface.m_connectedWindows.begin(); it!=ourInterface.m_connectedWindows.end(); ++it) {
		...
		// TODO Hauke: check epsilon of window here
	}
