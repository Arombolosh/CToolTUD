...
void SolarLoadsDistributionModel::initDefaults() {
	m_distributionType = NANDRAD::SolarLoadsDistributionModel::SWR_AreaWeighted;
	...
	m_para[P_RadiationLoadFractionFloor].set( KeywordList::Keyword("SolarLoadsDistributionModel::para_t", P_RadiationLoadFractionFloor),	 0.4, IBK::Unit("---"));
...
void SolarLoadsDistributionModel::checkParameters() const {
	...
	if (m_distributionType == SWR_SurfaceTypeFactor) {
		// check that all fractions add up to 1
		double fractionSum = m_para[P_RadiationLoadFractionZone].value  +
				m_para[P_RadiationLoadFractionFloor].value +
				m_para[P_RadiationLoadFractionWalls].value +
				m_para[P_RadiationLoadFractionCeiling].value;

		if (!IBK::near_equal(fractionSum, 1.0))
			throw IBK::Exception("Radiation load fractions for zone, walls, ceiling and floor must add up to 1.", FUNC_ID);
	}
}
