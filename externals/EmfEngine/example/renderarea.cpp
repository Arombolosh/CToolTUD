...
void RenderArea::draw(QPainter *painter)
{
    ...
    QRadialGradient radialGrad(QPointF(30, 40), 40, QPointF(50, 50));
	...
	conGrad.setColorAt(0, Qt::red);
	conGrad.setColorAt(1, Qt::black);
	//conGrad.setCoordinateMode(QGradient::StretchToDeviceMode);
	...
	for (int x = 0; x < width(); x += 100){
		for (int y = 0; y < height(); y += 100){
			...
			if (shape == RoundedRect && drawnRects < 3){
				if (drawnRects == 0)
					painter->setBrush(QBrush(linearGrad));
				else if (drawnRects == 1)
					painter->setBrush(QBrush(radialGrad));
				else if (drawnRects == 2)
